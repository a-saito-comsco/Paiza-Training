...
class Program
{
    ...
    internal static class Config
    {
        ...
        internal static Dictionary<int, List<(int, int)>> Lookup{ get; private set; } = new Dictionary<int, List<(int, int)>>();
        //internal static Dictionary<int, Dictionary<(int, int), bool>> OuterDictionary { get; private set; } = new Dictionary<int, Dictionary<(int, int), bool>>();
        ...
        internal static void GetInputs()
        {
            ...
            Lookup = AllRoads
                .Where(tapple => tapple.Value)
                .GroupBy(tapple => tapple.Key.Item1)
                .ToDictionary(
                    gr => gr.Key,
                    gr => gr.Select(tapple =>tapple.Key).ToList()
                );
        }
    }
    ...
    internal class Execution
    {
        ...
        private static bool FindPath(Dictionary<(int, int), bool> remRoads, int ruletRem, int current)
        {
            ...
            if (Config.Lookup.TryGetValue(current, out var keyList))
            {
